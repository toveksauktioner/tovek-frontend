...
function init() {
	try {
				...
				case 'update':
					...
					if( sMessage == 'New bid accepted' ) {
						...
						if( $(".view.bidList[data-item-id=" + iItemId + "]").length > 0 ) {
							...
							console.log("Update bid list - " + iItemId);
						...
						if( $(".view.bidFormAdd[data-item-id=" + iItemId + "]").length > 0 ) {
							...
							$.ajax( {
								url: "/?ajax=true&view=auction/bidFormAdd.php&itemId=" + iItemId + "&time=" + currentTimestamp,
								type: "GET",
								data: "noCss=true",
								async: true,
								dataType: "html"
						 ...
						 */
						$.ajax( {
							url: "/?ajax=true&view=auctionAjax/bidDataAjax.php&newBid=true&itemId=" + iItemId + "&time=" + currentTimestamp,
							type: "GET",
							data: "noCss=true",
							async: true,
							dataType: "html"
		...
		oWebsocket.addEventListener( 'close', function( event ) {
			...
			if( (bConnectionCheck === true) && (connectionLive === true) ) checkConnection();
		} );
