...
// Auction list slideshow 
// https://www.seancdavis.com/posts/simple-looping-crossfade-image-slideshow/
class Slideshow {
  constructor(slideshowObj) {
  	this.container = slideshowObj;
    this.initSlides();
    this.initSlideshow();
  }
  ...
  initSlides() {
    this.slides = this.container.find('a');
  ...
  performSlide() {
  	let thisObj = this;
    this.currentSlide.fadeOut(1500, function() {
    	$(this).removeClass('current');
		thisObj.setNextSlide();
    } );

  }
  // 1. Change which is current
  // 2. Change which is next
  setNextSlide() {
    this.currentSlide = this.container.find(`[data-slide="${this.currentIndex}"]`).first();
    ...
    this.nextSlide.addClass('next').fadeIn();
  }
