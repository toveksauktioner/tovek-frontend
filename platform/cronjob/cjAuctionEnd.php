...
print_r($aAuctionParts);
    ...
    // Exception logging
	file_put_contents( dirname(dirname(__FILE__)) . '/logs/cjAuctionEndErrors.log', date('Y-m-d H:i:s') . ' ' . sprintf( _('Exception: "%s" on line %s in file %s at code %s'), $oThrowable->getMessage(), $oThrowable->getLine(), $oThrowable->getFile(), $oThrowable->getCode() ), FILE_APPEND );
