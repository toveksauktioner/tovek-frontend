...
if( !empty($aPuffs) ) {
	...
	// if( !empty($aRouteToObject) ) {
  	...
  	// 	$aData = $oPuff->readByRoute( $oRouter->iCurrentRouteId, $aReadFields, arrayToSingle($aPuffs, null, 'puffId'), array( '(entPuff.puffLayoutKey != "" AND entPuff.puffLayoutKey = "' . "contactPuff" . '")' ) );
  	// } else {
  	// 	// Normal
  	// 	$aData = $oPuff->read( $aReadFields, arrayToSingle($aPuffs, null, 'puffId'), array( '(entPuff.puffLayoutKey != "" AND entPuff.puffLayoutKey = "' . "contactPuff" . '")' ) );
  	// }
	...
	if( !empty( $aData ) ) {
		...
		foreach( $aData as $iKey => $aEntry ) {
			if( empty( $aEntry['puffLayoutKey'] ) ) {
				$aEntry['puffLayoutKey'] = 'contactPuff';
			}
...
$oTemplate->addBottom( array(
	...
	'content' => '
	<script>
		$( ".titleContact" ).hover( function() {
			console.log("test");
			$( this ).find( ".contactInfo" ).addClass("show");
		}, function() {
			$( this ).find( ".contactInfo" ).removeClass("show");
		} );
	</script>'
) );
