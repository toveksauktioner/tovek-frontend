...
$oNavigation->setGroupKey( $GLOBALS['viewParams']['navigation']['list.php']['groupKey'] );
...
if( !empty($aTree) ) {
	...
	foreach( $aTree as $key => $entry ) {
		...
		$sOutput .= '<li' . ( !empty($aClass) ? ' class="' . implode(' ', $aClass) . '"' : '' ) . '><a href="' . $entry['navigationUrl'] .'" target="' . $sTarget . '" class="ajax">' . $entry['navigationTitle'] . '</a>';
