...
if( empty($GLOBALS['viewParams']['navigation']['listUser.php']['groupKey']) ) {
	$GLOBALS['viewParams']['navigation']['listUser.php']['groupKey'] = 'user';
}

$oNavigation = clRegistry::get( 'clNavigation', PATH_MODULE . '/navigation/models' );
$oNavigation->setGroupKey( $GLOBALS['viewParams']['navigation']['listUser.php']['groupKey'] );
...
if( !empty($aTree) ) {
    ...
    if( $entry['depth'] == 0 ) $iLastKey = $key;
  }
	reset($aTree);
	$iFirstKey = key($aTree);

	$sOutput .= '
		<ul class="navMain tabs">';
