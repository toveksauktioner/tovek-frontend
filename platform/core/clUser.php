...
class clUser {
	...
	public function __construct( $iUserId = null ) {
		$this->oDao = clRegistry::get( 'clUserDao' . DAO_TYPE_DEFAULT_ENGINE );
	...
	public function isOnline() {
		...
		// Set cookie for javascript to access data 
		// $iCookieExpire = time() + 3600;
		$aData = $this->readData( ['userId', 'username'] );
		setcookie( 'username', $aData['username'], 0, '/' );
		setcookie( 'userId', $aData['userId'], 0, '/' );
	...
	public function login( $sUsername, $sUserPass ) {
		...
		if( !empty($aData) ) {
			$iUserId = current(current($aData));
			$this->loginInit( $iUserId );

			// Set cookie for javascript to access data 
			// $iCookieExpire = time() + 3600;
			setcookie( 'username', $sUsername, 0, '/' );
			setcookie( 'userId', $iUserId, 0, '/' );
	...
	public function logout() {
		...
		// Delete cookie for javascript not to indicate logged in 
		// Time is passed so cookie will be deleted
		$iCookieExpire = time() - 3600;
