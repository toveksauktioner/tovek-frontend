...
class Compiler
{
    ...
    protected function compileChild($child, OutputBlock $out)
    {
        ...
        switch ($child[0]) {
            ...
            case Type::T_INCLUDE:
                ...
                if (! $mixin) {
                    // Try autoload
                    if( self::autoload( $child, $out ) ) {
                        $mixin = $this->get(self::$namespaces["mixin"] . $name, false);
        
                        if( !$mixin ) {
                            $this->throwError("File for mixin $name found, but it was not defined");
                        }
                    } else {
    ...
    public function autoload( $child, $out ) {	
		// Convert to import
		list(,$name) = $child;
		$rawPath = strtr( $name, '\\', '/' );
		$rawPath = $this->reduce($rawPath);
		...
		return $this->compileImport($importChild, $out);
	}
