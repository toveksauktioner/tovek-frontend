...
class PHPExcel_Writer_CSV extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	/**
	 * Fetch PHPExcel (Custom function by Argonova)
	 *
	 * @param	string		$pFilename
	 * @throws	PHPExcel_Writer_Exception
	 */
	public function fetch() {
		...
		if ($this->_excelCompatibility) {
			$sContent .= "\xEF\xBB\xBF";			//	Enforce UTF-8 BOM Header
			$this->setEnclosure('"');				//	Set enclosure to "
			$this->setDelimiter(";");			    //	Set delimiter to a semi-colon
            $this->setLineEnding("\r\n");
			$sContent .= 'sep=' . $this->getDelimiter() . $this->_lineEnding;
		} elseif ($this->_useBOM) {
			// Write the UTF-8 BOM code if required
		...
		for($row = 1; $row <= $maxRow; ++$row) {
			...
			if (is_array($cellsArray[0])) {
				...
				// Write to file
				$sContent .= $line;
				$aContent[] = substr( $line, 1, strlen($line) - 1 );
			} else {
