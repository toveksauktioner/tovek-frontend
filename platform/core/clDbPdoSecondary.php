...
class clDbPdoSecondary implements ifDb {
	...
	function __construct( $aParams = array() ) {
		$aParams += array(
			'database' => DB_SECONDARY_DATABASE,
			'engine' => DB_SECONDARY_ENGINE,
	...
	public function query( $sQuery ) {
		return $this->querySecondaryDb( $sQuery );
	}

	/**
	 * @see ifDb::query()
	 */
	public function querySecondaryDb( $sQuery ) {
	...
	public function write( $sQuery, $aParams = array() ) {
		return $this->writeSecondaryDb( $sQuery, $aParams );
	}

	/**
	 * @see ifDb::write()
	 */
	public function writeSecondaryDb( $sQuery, $aParams = array() ) {
		#echo $sQuery . '<br /><br />';
		//clFactory::loadClassFile( 'clLogger' );
